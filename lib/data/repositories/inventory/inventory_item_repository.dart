...
[ { "Item Code": "", "Description": "<text from ITEM DESCRIPTION>", "Qty": 1, "Unit": "UNIT", "Part No.": "<text from PART NO.>", "Serial No.": "<text from SERIAL NO.>", "PTN": "<text from PTN>", "batch": [] }, { "Item Code": "", "Description": "<text from ITEM DESCRIPTION>", "Qty": 1, "Unit": "PIECE", "Part No.": "", "Serial No.": "<text from SERIAL NO.>", "PTN": "", "batch": [] } ]''';
  ...
  Future<Result<List<InventoryItemModel>>> analyzeFile(File file) async {
    try {
      ...
      // Bake EXIF orientation into the pixels first: the OCR model reads raw
      // pixels, and a rotated table breaks column alignment. `prepare` also
      // reports the mime type describing the bytes it returns, which is what
      // the request must declare after any re-encode.
      final prepared = await BDocumentImage.prepare(file);
      final mimeParts = prepared.mimeType.split('/');
      ...
      final fileStream = http.MultipartFile.fromBytes(
        'imageFile',
        prepared.bytes,
        filename: BDocumentImage.fileNameOf(file.path),

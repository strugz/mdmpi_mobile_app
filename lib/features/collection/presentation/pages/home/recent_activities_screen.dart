          ...
          Expanded(
            child: Obx(() {
              ...
              if (recentItems.isEmpty) {
                return const Center(
                    child: Text('No engagement history found.'));
              }
              ...
              final filteredItems = controller.activityFilter.value == 'All'
                  ...
                  : recentItems
                      .where((item) =>
                          item['history'].status ==
                          controller.activityFilter.value)
                      .toList();

              if (filteredItems.isEmpty) {
                return const Center(
                    child: Text('No activities match this filter.'));
              }

              final historyList = filteredItems
                  .map((e) => e['history'] as CollectionHistoryModel)
                  .toList();
              final accountNames = {
                for (var i = 0; i < filteredItems.length; i++)
                  i: filteredItems[i]['accountName'].toString()
              };
              final invoiceIds = {
                for (var i = 0; i < filteredItems.length; i++)
                  i: filteredItems[i]['invoiceId']?.toString()
              };
              final items = {
                for (var i = 0; i < filteredItems.length; i++)

...
class StandardDeliveryDataManager {
  final StandardDeliveryRepository _repository = Get.find<StandardDeliveryRepository>();
  final CancelRemarksRepository _cancelRemarksRepository = Get.find<CancelRemarksRepository>();
  final DatabaseHelper _dbHelper = DatabaseHelper.instance;
  final MessagingController _messageController = Get.find<MessagingController>();
  final WebSocketNotificationController _webSocketController = Get.find<WebSocketNotificationController>();
  ...
  Future<void> saveRequestFromForm(StandardDeliveryController controller) async {
    ...
    try {
      ...
      final normalizedFormCategory = ensureCategoryId(
          formState.formCategory, formState.formCategories);
      final normalizedItemCategory = ensureCategoryId(
          formState.itemCategory, formState.itemCategories);
      ...
      // Reload requests
      await fetchStandardDeliveryRequests(controller, controller.useLocalStorage.value);
  ...
  Future<void> cancelRequestWithRemarks(
      StandardDeliveryModel request, String remarks, String user, StandardDeliveryController controller) async {
    BFullScreenLoader.openLoadingDialog(
        'Saving on process...', BImages.docerAnimation);
  ...
  Future<void> fetchStandardDeliveryRequests(
      StandardDeliveryController controller, [bool useLocalStorage = true]) async {
    ...
    try {
      ...
      if (!useLocalStorage) {
        // Force API fetch
        logDebug('StandardDeliveryDataManager: Fetching from API (useLocalStorage=false)');
        ...
        if (results.isEmpty) {
          logDebug('StandardDeliveryDataManager: Local DB empty, fetching from API');
          final apiRequests = await _repository.getAllPending();
          results = apiRequests;
          await _dbHelper.insertRequests(apiRequests);
        } else {
          logDebug('StandardDeliveryDataManager: Loaded ${results.length} items from local DB');
        }
      }

      controller.allPendingRequests.assignAll(results);
      logDebug('StandardDeliveryDataManager: Assigned ${results.length} requests to controller');

      controller.filterManager.applyFilter(controller.allPendingRequests.toList());
      controller.updateRequestCounts();
    } catch (e) {
      controller.errorMessage.value = e.toString();
      logDebug('StandardDeliveryDataManager.fetchStandardDeliveryRequests error: $e');
  ...
  Future<CancelRemarksModel> fetchCancelRemarks(String requestId) async {
    try {
      logDebug('🔍 StandardDeliveryDataManager: Fetching cancel remarks for: $requestId');
      ...
      try {
        ...
        if (exists) {
          ...
          logDebug('✅ StandardDeliveryDataManager: Found local remarks: "${localRemarks.remarks}"');
          return localRemarks;
        }
      } catch (e) {
      ...
      try {
        ...
        logDebug('✅ StandardDeliveryDataManager: API returned remarks: "${result.remarks}" date: "${result.date}"');
        ...
        if (result != CancelRemarksModel.empty) {
          ...
          try {
            ...
            logDebug('💾 StandardDeliveryDataManager: Persisted remarks to local DB');
          } catch (e) {
            logDebug('⚠️ StandardDeliveryDataManager: Failed to persist remarks: $e');
          }
          return result;
        }

...
/// Developer-facing widget that lists pending signature upload outbox rows.
class SignatureOutbox extends StatelessWidget {
  const SignatureOutbox({super.key});

  @override
  Widget build(BuildContext context) {
    final controller = Get.find<SignatureOutboxController>();
            ...
            SliverPadding(
              ...
              sliver: SliverList(
                delegate: SliverChildListDelegate([
                  ...
                  Text(
                    'Retry failed uploads, inspect stored signatures, or clear developer outbox entries.',
              ...
              SliverPadding(
                ...
                sliver: SliverList.separated(
                  ...
                  itemBuilder: (_, index) {
                    ...
                    return _SignatureOutboxCard(
                      item: item,
                      isBusy: controller.isBusy(item.requestId),
  ...
  Future<void> _confirmIgnore(
    ...
    SignatureOutboxController controller,
    ...
    final shouldIgnore = await showDialog<bool>(
      ...
      builder: (dialogContext) {
        return AlertDialog(
          ...
          content: Text(
            'Remove signature outbox entry for request ${item.requestId}? This action is intended for developer cleanup only.',
          ),
          actions: [
            TextButton(
  ...
  void _showPreviewDialog(BuildContext context, SignatureOutboxItem item) {
    showDialog<void>(
      context: context,
      builder: (dialogContext) {
        return Dialog(
...
class _SummaryCard extends StatelessWidget {
  ...
  final SignatureOutboxController controller;
...
class _BulkActions extends StatelessWidget {
    ...
    final shouldRetry = await showDialog<bool>(
      ...
      builder: (dialogContext) {
        return AlertDialog(
          ...
          content: const Text(
            'Retry every pending signature upload in the outbox?',
          ),
          actions: [
            TextButton(
              onPressed: () => Navigator.of(dialogContext).pop(false),
  ...
  Future<void> _confirmClearAll(
    ...
    final shouldClear = await showDialog<bool>(
      ...
      builder: (dialogContext) {
        return AlertDialog(
          title: const Text('Clear Signature Outbox'),
          content: const Text(
            'Remove all pending signature outbox entries? This developer action cannot be undone.',
          ),
          actions: [
            TextButton(
              onPressed: () => Navigator.of(dialogContext).pop(false),
...
class _SignatureOutboxCard extends StatelessWidget {
  const _SignatureOutboxCard({
    required this.item,
  ...
  final SignatureOutboxItem item;
  final bool isBusy;
              ...
              children: [
                ClipRRect(
                  ...
                  child: SizedBox(
                    width: 112,
                    height: 72,
                    child: _SignaturePreview(signatureBase64: item.signatureBase64),
                  ),
                ),
...
class _SignaturePreview extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final bytes = _decodeBase64(signatureBase64);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: BSizes.spaceBtwItems),
            Text(
              'No pending signature uploads',
              style: Theme.of(context).textTheme.titleLarge,
              textAlign: TextAlign.center,
            ),
            const SizedBox(height: BSizes.xs),
            Text(
              'Captured signatures are either synced already or there are no outbox rows to review.',
              style: Theme.of(context).textTheme.bodyMedium,
              textAlign: TextAlign.center,
            ),
          ],

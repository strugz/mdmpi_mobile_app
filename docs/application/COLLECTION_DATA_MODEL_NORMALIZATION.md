...
| **D. Drop redundancy** ✅ DONE 2026-09-15 (`migration_20260915_collection_normalization_stage_d.sql`; 154 tests; plan: `COLLECTION_NORMALIZATION_STAGE_D_PLAN.md`) | Drop the 5 client-snapshot columns and `bankname` from the invoice, and `bankname/checkno/checkdate` from the engagement; add the `clientcode` FK; keep `bpcode`, `collectorname` and the cache columns (recompute function added); keep the identity PK. | Schema only; contract unchanged |

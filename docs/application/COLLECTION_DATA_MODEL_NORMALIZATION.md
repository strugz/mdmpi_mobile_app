...
| **D. Drop redundancy** | Remove the 8 client-snapshot columns and `bankname` from the invoice; make `balance`/`totalcollected` cache-only or drop; `documentid` becomes the PK. | Schema only; contract unchanged |

...
--
-- Added 2026-09-03 (migration_20260903_add_a_tblloseitem.sql):
-- Name: a_tblloseitem; Type: TABLE; Schema: public; Owner: postgres
-- Lost items on Pull Out / Return requests (item not pulled out + remarks).
--

CREATE TABLE public.a_tblloseitem (
...
CREATE INDEX idx_a_tblloseitem_requestid ON public.a_tblloseitem (requestid);

--
-- Added 2026-09-04 (migration_20260904_add_a_tblrequestpickupitemcategory.sql):
-- Name: a_tblrequestpickupitemcategory; Type: TABLE; Schema: public; Owner: postgres
-- Multi-category Pick Up: full selection per request (scalar itemcategoryid
-- on a_tblrequestpickupmdmpi keeps the primary/first selection).
--

CREATE TABLE public.a_tblrequestpickupitemcategory (
...
CREATE INDEX idx_a_tblrequestpickupitemcategory_requestid
    ON public.a_tblrequestpickupitemcategory (requestid);

--
-- Added 2026-09-07 (migration_20260907_add_a_tblbackloaditem.sql):
-- Name: a_tblbackloaditem; Type: TABLE; Schema: public; Owner: postgres
-- Backloaded items on Standard Delivery / Hotline Direct requests (item not
-- received by the client at drop off + remarks). Distinct from
-- a_tblrequestbackload, which records whole-request backload events.
--

CREATE TABLE public.a_tblbackloaditem (
    backloaditemid bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    requestid bigint NOT NULL,
    itemcode character varying(100),
    remarks character varying(500),
    createdat timestamp without time zone,
    CONSTRAINT fk_a_tblbackloaditem_request FOREIGN KEY (requestid)
        REFERENCES public.a_tblrequeststandarddelivery (requestid)
        ON DELETE CASCADE
);

ALTER TABLE public.a_tblbackloaditem OWNER TO postgres;

CREATE INDEX idx_a_tblbackloaditem_requestid ON public.a_tblbackloaditem (requestid);

...
- Bottom-anchored UI (Android runs edge-to-edge, so the navigation bar / gesture pill overlaps every screen's bottom): pad for the navigation bar with `SafeArea(top: false)` or `BDevicesUtils.systemBottomInset(context)` exactly once, at the outermost bottom widget — never around a whole `Scaffold` (it shortens maps and camera previews). Pad for the keyboard with `viewInsets` exactly once (`Scaffold` already does it for `bottomNavigationBar`; sheet presenters add it themselves). Never add the two together and never use `viewInsets.bottom > 0` to detect "gesture navigation" — it is the keyboard; `test/common/widgets/bottom_inset_test.dart` fails on that pattern.
...
- Inventory OCR / AI: `lib/data/repositories/inventory/inventory_item_repository.dart` calls Google Generative Language directly (`analyzeFileWithGemini()`), reading `.env` keys `AI_TOOLKIT_MODEL`, `AI_TOOLKIT_API_KEY`, and optional `AI_PROMPT` (with fallbacks to `AI_MODEL` / `API_KEY`); `StandardDeliveryController.analyzeFileWithAiToolkit` is the single entry point for scanned inventory intake. The former backend proxy path (`analyzeFile()` → `/api4/Gemini/analyze-file`) and its `GeminiController` were removed on 2026-09-10 as dead code.
